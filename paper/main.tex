...
exactly this capability. Synthetic recall suites such as MAD \citep{poli2024mad}
and Zoology \citep{arora2023zoology} exist precisely to \emph{select} among these
...
is exactly the one least likely to survive the change in schedule, budget, and
...
empirical claim on its own terms. It is a good test case precisely because it looks
...
\item \textbf{The schedule confound} (\S\ref{sec:schedule}). Training is a
  ...
  4{,}000 steps lifts the \emph{baseline} from 3/16 to 11/16 solved \emph{at step
  2{,}000}---the same training compute (McNemar $p\approx0.008$). A constant
  learning rate alone reaches 8/16 --- we cannot distinguish it from \ns\
...
\item \textbf{Mechanism by elimination} (\S\ref{sec:mechanism}). The residual
  ...
  erase-before-write keeps the memory perfectly conditioned (effective rank 9.9
...
\item \textbf{The scaffold property} (\S\ref{sec:scaffold}). Read-time
  ...
  12/16 seeds finish as \emph{numerically stock mLSTMs} at 95--100\% accuracy.
  Gating the anneal on escape rather than a fixed window recovers the late
  escaper: 13/16 solved --- matching the un-annealed combination exactly --- with
...
\item \textbf{Where the scaffold matters} (\S\ref{sec:basin}). On a 144-run grid
  ...
  edge. The baseline's hazard shrinks with hardness, so the \emph{realized} gap
...
\item \textbf{The escape law} (\S\ref{sec:batch}). Across a batch-size $\times$
  ...
  required large batch as a \emph{noise} condition, not a capacity one.
...
heads, pre/post LayerNorm, $\sim$78K parameters, exactly the reference
...
seq 512, cosine $T_{\max}{=}2000$ except where noted; 16 seeds each). The
orthogonalized read (\ns) triples the solved fraction over the baseline --- but
deleting the LR decay (constLR) recovers most of that gap for free, and the two
levers together (\ns{}+constLR) dominate. Curves are Kaplan--Meier estimates of the fraction of
seeds escaped by each step.}
\label{fig:survival}
\end{figure}
...
\caption{The schedule alone flips half the seeds. Baseline mLSTM, v80/s512, same
16 seeds per curve. Stretching the cosine period to $T_{\max}{=}4000$ lifts the
baseline from 3/16 to \textbf{11/16 solved by step 2{,}000} --- identical
training compute, McNemar $p\approx0.008$. Escape requires the learning rate to
still be hot when the run is ready to transition; aggressive decay strangles it.}
\label{fig:schedule}
\end{figure}
...
classic grokking is delayed \emph{generalization} after the training set has been
...
budget with a decaying learning rate therefore \emph{censors} the escape process
twice: runs that would escape later are cut off by the budget, and---more
insidiously---the decay itself removes the optimizer heat that escape requires.
...
\textbf{11/16 by step 2{,}000}---the very step count at which the original schedule
...
ratio at $1.5$ with 95\% CI $[0.6, 4.3]$ --- 16 seeds per arm leave effects up to
${\sim}4\times$ in either direction compatible, so this is a statement of
non-detection, not equivalence. What 16 seeds \emph{do} establish is the
per-compute ordering at this hardness: \ns's per-position Newton--Schulz graph
costs ${\sim}6\times$ baseline wall time per step, and a wall-matched arm makes
the consequence concrete --- given \ns's ${\sim}12$-minute budget (12{,}000
constant-LR steps), the plain baseline solves \textbf{14/16}, above \ns's 9/16
and \ns{}+constLR's 13/16 at the same wall cost (Fig.~\ref{fig:compute}). At the
...
\caption{The escape comparison per unit compute, plotted against each run's
\emph{measured} wall-clock on identical L4 GPUs. Each family appears under its
strongest known schedule recipe --- baseline under cosine $T_{\max}{=}4000$ and
under constant LR with a wall-matched 12{,}000-step budget; \ns\ under its
published schedule and with the constant-LR lever added --- because the practical
question is what to run under a fixed wall-clock budget; pairing \ns\ against
weaker baseline schedules would only flatter it. Given the ${\sim}12$ minutes
\ns\ needs to solve 9/16, the wall-matched constant-LR baseline solves 14/16,
most of them within its first four minutes.}
\label{fig:compute}
\end{figure}
...
vs.\ 0.42 for \ns\ --- the whitened read \emph{doubles} read-path gradient
...
\caption{Memory rank collapse is a symptom, not the cause. Effective rank of the
raw memory $\C$ (final position, first validation batch) over training; one line
per seed, colored by outcome. Baseline and \ns\ escapes coincide with rank
recovery --- but \textsc{delta}'s erase-before-write holds effective rank
${\sim}10$ throughout and still fails (2/16). Conditioning the \emph{state} is not
the lever; conditioning the \emph{read during learning} is.}
\label{fig:rank}
\end{figure}
...
baseline mLSTM at 3/16). Normalization alone ($k{=}0$) and a single iteration
confer nothing; three iterations recover nearly all of the read-path gradient
coherence and most of the benefit at 70\% of \ns's wall time.}
\label{fig:dose}
\end{figure}
...
iteration lands \emph{below} the baseline (0/16 vs.\ 3/16), with plateau
...
optimization problem, and the optimizer must actually optimize \emph{that}
...
probe: the recurrently computed regularized least-squares read is exactly the
...
Finally, read-path gradient coherence is a \emph{thermometer, not a thermostat}:
...
\emph{when} a seed escapes (all $p>0.2$). Elevated coherence is the shared
...
borderline seed, at 78\%, edges to 82\%) --- the benefit is not read-time
...
only \emph{after} a run has already escaped within the nominal budget, so the
extension cannot manufacture solves --- it exists solely to complete the
scaffold's removal, and given \S\ref{sec:schedule} we flag it explicitly: any
...
\caption{Scaffold anneal (fixed window, shaded): the orthogonalized read is
interpolated back to the raw read over steps 1400--2000. Twelve of 16 seeds end
the run as numerically stock mLSTMs at 95--100\% accuracy. The gray late escaper
caught mid-anneal motivates the escape-gated variant.}
\label{fig:anneal}
\end{figure}
...
competing risk distinct from plateauing). The baseline's workable-LR corridor
narrows as the task hardens; the whitened read widens it in both directions and
lifts its center (24/24 at $3{\times}10^{-3}$).}
\label{fig:basin}
\end{figure}

The elimination table pins \emph{what} the residual effect is; the basin map pins
\emph{where it matters}. We ran constant LR
...
\emph{destabilize} at $10^{-2}$ (19/24 collapse below chance across regimes) while
...
$3{\times}10^{-3}$ is $4.9\times$ but at $10^{-2}$ is $0.23\times$ --- \emph{plus}
the competing destabilization risk. A pure cumulative-heat law
($\int \mathrm{lr}\,dt$) fails: v80 escapes at $10^{-3}$ on \emph{less} total heat
...
regime. The realized advantage grows with hardness exactly as the
...
row) so that small-batch cells are not trivially censored; the hazard model
below accounts for the differing budgets through censoring. The workable
corridor sits at $10^{-3}$ for batch $\le$16 and moves to $3{\times}10^{-3}$ at
batch 32--64; $10^{-2}$ destabilizes 23/24 runs at batch $\le$16 but becomes
survivable at batch 64.}
\label{fig:batch}
\end{figure}
...
$\log \mathrm{lr}$ at fixed noise, and $\log(\mathrm{lr}^2/B)$, the standard
...
\emph{is} the corridor of \S\ref{sec:basin} --- its cold edge is
...
($p=0.017$). The scaffold rescues exactly the cells that shrinking batch
...
own cosine schedule at v96/s768, batch 64 takes the \emph{baseline} from
...
token from the feature $\kk_u^{\!\top}\C_t$ --- the model's \emph{own} stored key
at the pair's write position, slicing its \emph{own} memory at query time ---
...
with the key from the pair's \emph{value} position (where the causal convolution
...
\item \textbf{Fix the schedule before adding machinery.} On plateau-escape tasks,
  ...
  study's own budget, at $1/6$ the per-step cost --- and per unit wall-clock the
...
solve are different test-time optimizers, and they behave differently in our table
--- but what predicts \emph{trainability} of this task is not any single axis; it
...
\emph{optimizer} design: Muon \citep{jordan2024muon,bernstein2024old} orthogonalizes
